<?php
// Template downloads must run before any output to avoid header errors.
...
// CSV upload: lecturers
if (isset($_POST['action']) && $_POST['action'] === 'csv_lec') {
        ...
        if ($missing) {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                if (!$lid || !$lname || !$lpass || !$lemail) {
                    $csvLog[] = ['row' => $rowNum, 'id' => $lid ?: '(empty)', 'status' => 'error', 'msg' => 'Missing required field (id, name, email, or password)'];
                    $errors++; continue;
                }
            ...
            fclose($handle);
            $msg = "Lecturer CSV: {$added} added, {$skipped} skipped, {$errors} error(s).";
        }
    } else {
...
// CSV upload: supervisors
if (isset($_POST['action']) && $_POST['action'] === 'csv_sup') {
    if (!empty($_FILES['csv_file']['tmp_name'])) {
        $handle = fopen($_FILES['csv_file']['tmp_name'], 'r');
        ...
        if ($missing) {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                if (!$sid || !$sname || !$spass || !$semail || !$cname) {
                    $csvLog[] = ['row' => $rowNum, 'id' => $sid ?: '(empty)', 'status' => 'error', 'msg' => 'Missing required field (id, name, email, password, or company)'];
                    $errors++; continue;
                }

                // Resolve company_id from company_name; create the company if it does not exist
                $cid_val = 'NULL';
                ...
                if ($cRow = mysqli_fetch_assoc($cRes)) {
                    $cid_val = (int)$cRow['company_id'];
                } else {
                    mysqli_query($conn, "INSERT INTO company (company_name) VALUES ('$cn')");
                    $cid_val = mysqli_insert_id($conn);
                }
            ...
            fclose($handle);
            $msg = "Supervisor CSV: {$added} added, {$skipped} skipped, {$errors} error(s).";
        }
    } else {
        $msg = "ERROR: No file uploaded.";
    }
...
// Load record for edit mode if an edit ID is provided
...
<div class="topbar">
    ...
    <div class="tab-btn-group">
        <button class="btn tab-btn active" onclick="switchTab('lecturers', this)">🧑‍🏫 Manage Lecturers</button>
        <button class="btn tab-btn" onclick="switchTab('supervisors', this)">👔 Manage Supervisors</button>
    </div>
</div>
...
<!-- Lecturers tab panel -->
<div id="tab-lecturers" class="tab-panel">
...
</div><!-- end tab-lecturers -->

<!-- Supervisors tab panel -->
<div id="tab-supervisors" class="tab-panel" style="display:none;">
<!-- Supervisors -->
<div class="section-label-row">👔 Supervisors</div>

<!-- Supervisor CSV upload -->
<div class="card">
    <div class="card-title flex-between">
