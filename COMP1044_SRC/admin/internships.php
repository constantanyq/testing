<?php
// TEMPLATE DOWNLOAD
if (isset($_GET['template']) && $_GET['template'] === 'company') {
    header('Content-Type: text/csv; charset=utf-8');
    header('Content-Disposition: attachment; filename="company_template.csv"');
    header('Cache-Control: no-cache, no-store, must-revalidate');
    $out = fopen('php://output', 'w');
    fputs($out, "\xEF\xBB\xBF"); // BOM for Excel UTF-8 compatibility
    fputcsv($out, ['company_name']);
    fputcsv($out, ['Petronas Berhad']);
    fputcsv($out, ['Maybank Berhad']);
    fputcsv($out, ['Telekom Malaysia Berhad']);
    fclose($out);
    exit();
}
...
if (isset($_POST['action']) && $_POST['action'] === 'csv_company') {
    $csvLog = [];
    if (!empty($_FILES['csv_file']['tmp_name'])) {
        ...
        if (!in_array('company_name', $header)) {
            $msg = "ERROR: CSV must have a 'company_name' column.";
        } else {
            $added = $skipped = 0;
            $rowNum = 1;
            while (($row = fgetcsv($handle)) !== false) {
                ...
                $cname = mysqli_real_escape_string($conn, trim($data['company_name'] ?? ''));
                if (!$cname)
                    continue;
            ...
            fclose($handle);
            $msg = "Company CSV: {$added} added, {$skipped} skipped.";
        }
    } else {
...
while ($r = mysqli_fetch_assoc($svRes))
    $allSupervisors[] = $r;
...
function getLecturerLabel($lecturers, $id)
{
    foreach ($lecturers as $l) {
        if ($l['lecturer_id'] == $id)
            return $l['lecturer_name'] . ' (' . $l['lecturer_id'] . ')';
    }
    return '';
}
function getSupervisorLabel($supervisors, $id)
{
    foreach ($supervisors as $s) {
        if ($s['supervisor_id'] == $id)
            return $s['supervisor_name'] . ' (' . $s['supervisor_id'] . ')';
    }
    return '';
}
...
<div class="page-body">
    ...
    <div class="card" data-section="company-form">
        ...
        <div class="csv-info-box">
            ...
            <div>Bulk-add companies from a CSV file. Required column: <span class="csv-col">company_name</span>.</div>
        </div>
        <form method="POST" enctype="multipart/form-data" style="margin:14px 0 20px;">
            <input type="hidden" name="action" value="csv_company">
            <div class="csv-upload-area" id="csvDropCompany">
                <input type="file" name="csv_file" id="csvFileCompany" accept=".csv" required style="display:none;">
                <div class="csv-upload-icon">📁</div>
                <p class="csv-upload-label">Drag & drop your CSV file here, or <span class="csv-browse-link"
                        onclick="document.getElementById('csvFileCompany').click()">browse</span></p>
                <p class="csv-upload-hint" id="csvFileNameCompany">Accepted format: .csv only</p>
            </div>
            <div style="margin-top:12px;"><button type="submit" class="btn btn-success">📤 Upload Companies</button>
            </div>
        </form>
        ...
        <p
            ...
            <?= $editCompany ? '✏️ Edit Company' : '➕ Add Single Company' ?>
        </p>
                    ...
                    while ($crow = mysqli_fetch_assoc($cListRes)):
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                $warnParts = [];
                                if ($crow['sup_count'] > 0)
                                    $warnParts[] = $crow['sup_count'] . ' supervisor(s)';
                                if ($crow['int_count'] > 0)
                                    $warnParts[] = $crow['int_count'] . ' internship(s)';
        ...
        <form method="POST" id="internshipForm">
            ...
            <?php if (!$editRow): ?>
                <div class="form-group">
                    ...
                    <div class="search-dropdown" id="sd-student">
                        <input type="text" class="sd-input" placeholder="Type student name or ID to search..."
                            autocomplete="off">
            ...
            <?php else: ?>
                <div class="form-group">
                    ...
                    <input type="text" value="<?= htmlspecialchars($editRow['student_id']) ?>" readonly
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div class="search-dropdown" id="sd-company">
                        <input type="text" class="sd-input" placeholder="Type company name to search..."
                            autocomplete="off"
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div class="search-dropdown" id="sd-lecturer">
                        <input type="text" class="sd-input" placeholder="Type lecturer name to search..."
                            autocomplete="off"
                ...
                <div class="form-group">
                    ...
                    <div class="search-dropdown" id="sd-supervisor">
                        <input type="text" class="sd-input" placeholder="Select a company first..." autocomplete="off"
                ...
                ORDER BY i.internship_id";
                    ...
                    while ($row = mysqli_fetch_assoc($res)):
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="internships.php?edit=<?= $row['internship_id'] ?>"
