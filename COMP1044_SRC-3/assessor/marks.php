...
$students = [];
while ($r = mysqli_fetch_assoc($studentsRes))
    $students[] = $r;
    ...
    <?php if ($selectedStudent): ?>
        <div class="card">
            ...
            <div class="alert alert-info" style="font-size:12px;">
                💡 Enter a mark out of <strong>100</strong> for each component. The weighted score is calculated
                automatically.
                        ...
                        <tbody>
                            <?php foreach ($components as $i => $comp):
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="number" class="mark-input" name="marks[<?= $cid ?>]"
                                            value="<?= htmlspecialchars($prevMark) ?>" min="0" max="100" step="0.5"
                                            data-weight="<?= $comp['component_weightage'] ?>" required>
                                    </td>
                                    <td class="total-cell">
                                        <?= $prevMark !== '' ? number_format((float) $prevMark * $comp['component_weightage'] / 100, 2) : '0.00' ?>
                                    </td>
                                    <td>
                                        <input type="text" name="comments[<?= $cid ?>]"
                                            value="<?= htmlspecialchars($prevCmt) ?>" placeholder="Optional comment..."
