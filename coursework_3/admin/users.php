...
<div class="page-body">
    ...
    <!-- ========== LECTURERS ========== -->
 <div id="section-lec" style="<?= $editSup ? 'display:none;' : '' ?>">
                ...
                <form method="POST">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="lecturer_id" value="<?= htmlspecialchars($editLec['lecturer_id'] ?? $nextLecId) ?>" <?= $editLec ? 'readonly' : '' ?> required>
                            <?php if (!$editLec): ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="lecturer_name" value="<?= htmlspecialchars($editLec['lecturer_name'] ?? '') ?>" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Email</label>
                            <input type="email" name="lecturer_email" value="<?= htmlspecialchars($editLec['lecturer_email'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Password</label>
                            <input type="text" name="lecturer_password" value="<?= htmlspecialchars($editLec['lecturer_password'] ?? '') ?>" required>
                        </div>
                    </div>
                    <div style="display:flex; gap:10px;">
        ...
        <div class="section-gap"><span>Lecturer Records</span></div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="users.php?del_lec=<?= urlencode($row['lecturer_id']) ?>" class="btn btn-danger btn-sm confirm-delete" data-msg="Delete lecturer <?= htmlspecialchars($row['lecturer_name']) ?>?">🗑️ Delete</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
    ...
    <!-- ========== SUPERVISORS ========== -->
    <div id="section-sup" style="<?= $editSup ? '' : 'display:none;' ?> margin-top: 48px;">
        <div class="card card-form">
                ...
                <form method="POST">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="supervisor_id" value="<?= htmlspecialchars($editSup['supervisor_id'] ?? $nextSupId) ?>" <?= $editSup ? 'readonly' : '' ?> required>
                            <?php if (!$editSup): ?>
                        ...
                        <div class="form-group">
                            <label>Full Name</label>
                            <input type="text" name="supervisor_name" value="<?= htmlspecialchars($editSup['supervisor_name'] ?? '') ?>" required>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Email</label>
                            <input type="email" name="supervisor_email" value="<?= htmlspecialchars($editSup['supervisor_email'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Password</label>
                            <input type="text" name="supervisor_password" value="<?= htmlspecialchars($editSup['supervisor_password'] ?? '') ?>" required>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="supervisor_company_id">
                            ...
                            ?>
                            <option value="<?= $cr['company_id'] ?>" <?= ($editSup && $editSup['company_id'] == $cr['company_id']) ? 'selected' : '' ?>>
        ...
        <div class="section-gap"><span>Supervisor Records</span></div>
        <div class="card card-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="users.php?del_sup=<?= urlencode($row['supervisor_id']) ?>" class="btn btn-danger btn-sm confirm-delete" data-msg="Delete supervisor <?= htmlspecialchars($row['supervisor_name']) ?>?">🗑️ Delete</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
...
<script>
    function toggleUserView(view) {
        const lec = document.getElementById('section-lec');
        const sup = document.getElementById('section-sup');
        const bLec = document.getElementById('btn-lec');
        const bSup = document.getElementById('btn-sup');

        if (view === 'lec') {
            lec.style.display = 'block';
            sup.style.display = 'none';
            bLec.className = 'btn btn-primary';
            bSup.className = 'btn btn-outline';
        } else if (view === 'sup') {
            lec.style.display = 'none';
            sup.style.display = 'block';
            bLec.className = 'btn btn-outline';
            bSup.className = 'btn btn-primary';
        }
    }

    // Logic to keep the correct section open after an Edit refresh
    <?php if ($editSup): ?>
        toggleUserView('sup');
    <?php endif; ?>
</script>
